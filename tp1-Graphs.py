...
def dijkstra(world):
    ...
    while frontier:
        ...
        t1 = time.time()
        # print(t1 - t0)
        successor = expand(node, idx, cost_now, explored)
        # print(time.time() - t1)
...
def expand(node, indices, cost_now, explored):
    children = []
    for idx in indices:
        for possible_move in range(1, 5):
            ...
            if movement is not None:
                ...
                # print(time.time() - t8)
                # if new_node.map not in explored:
                #     children.append(new_node)
    return children
...
def main():
    init = time.time()
    global world
    world = Container('entrada_3')
    node, cost_now, origin = dijkstra(world)
